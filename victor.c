...
// Função para localizar quartos disponiveis
...
// Função para gerar um codigo para a estadia
...
// Função secundaria para gerar o totais de diarias
...
//Procedimento para alocação  de quartos
...
//Função para cadastrar a estadia
void cadastraEstadia(FILE *arquivoEstadias, FILE *arquivoQuartos) {
  estadia e;
  quarto *quartos = NULL;
  int numQuartos = 0;

  carregarQuartos(arquivoQuartos, &quartos, &numQuartos);
  ...
  int quartoIndex =localizarQuartoDisponivel(quartos, numQuartos, e.Qhospedes,2);
  if (quartoIndex == -1) {
    ...
    e.numeroQuarto = quartos[quartoIndex].numero;
  ...
  if (scanf("%d %d %d", &e.dataSaida.dia, &e.dataSaida.mes, &e.dataSaida.ano) !=3) {
  ...
  e.codigoEstadia = proximo_codigo_estadia(arquivoEstadias);
